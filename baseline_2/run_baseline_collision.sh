benchmark="baseline_collision"  # benchmark name
...
file_override=0
file_override_name=""
while getopts h:p:m:t:s:r:f: flag
do 
    case "${flag}" in
...
for i in {1..8}
do
    ./run_benchmark.sh -m $node -p $probe -t $thread_count -c $i -s $shift_size -r $repeats -f $filename -h $hash_table_size;
done
